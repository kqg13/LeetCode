# #547: Number of Provinces

# There are n cities. Some of them are connected, while some are not. If city a is connected
# directly with city b, and city b is connected directly with city c, then city a is connected
# indirectly with city c.
#
# A province is a group of directly or indirectly connected cities and no other cities outside of the group.
#
# You are given an n x n matrix isConnected where isConnected[i][j] = 1 if the ith city and the jth city
# are directly connected, and isConnected[i][j] = 0 otherwise.
#
# Return the total number of provinces.

# Example:
# isConnected1 = [[1, 1, 0], [1, 1, 0], [0, 0, 1]] ---> 2
# isConnected2 = [[1, 0, 0], [0, 1, 0] ,[0, 0, 1]] ---> 3


class Solution(object):
    def findCircleNum(self, isConnected):
        """
        :type isConnected: List[List[int]]
        :rtype: int
        """
        nComponents, n = 0, len(isConnected)
        visited = [False] * n
        for i in range(n):
            if not visited[i]:
                nComponents += 1
                self.findCircleNumDFS(isConnected, i, n, visited)
        return nComponents

    def findCircleNumDFS(self, isConnected, node, n, visited):
        visited[node] = True
        for i in range(n):
            if i == node:
                continue
            if isConnected[node][i] == 1 and not visited[i]:
                self.findCircleNumDFS(isConnected, i, n, visited)


isConnected1 = [[1, 1, 0], [1, 1, 0], [0, 0, 1]]
isConnected2 = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
s = Solution()
print(s.findCircleNum(isConnected1))
print(s.findCircleNum(isConnected2))
